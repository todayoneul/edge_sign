...
> 📢 **데모 품질 업데이트(v3)**: 위 양자화 연구(섹션 1~9) 이후 **신호등 분리 검출 · 한국어 표지/신호등 인식 · 범용 실시간 입력**을 추가하였다 → [★ 최신 업데이트 섹션](#-최신-업데이트--v3-신호등-분리--한국어-인식--범용-입력-2026-05-31)
...
- [★ 최신 업데이트 — v3: 신호등 분리 · 한국어 인식 · 범용 입력](#-최신-업데이트--v3-신호등-분리--한국어-인식--범용-입력-2026-05-31)

---

## ★ 최신 업데이트 — v3: 신호등 분리 · 한국어 인식 · 범용 입력 (2026-05-31)

Phase 1·2의 양자화 연구(아래 섹션 1~9) 위에, **실시연(데모) 품질**을 끌어올리는 v3 작업을 진행하였다.
세 가지 축 — ① 검출기 신호등 분리 재학습, ② 한국 표지판/신호등 분류기 신규 학습, ③ 어떤 입력이든 받는 범용 실시간 파이프라인 — 이 모두 적용되어 main에 병합되었다.

![v3 검출 샘플](assets/v3/v3_detection_sample.jpg)

> 청량리역 사거리(주간) 한 프레임. **녹색 박스 = 교통표지판**(규제·지시·주의), **빨강 박스 = 신호등**(빨강·노랑 색상까지 분류). 라벨은 전부 한국어로 렌더링된다. *(학습 미사용 test 시퀀스 — 정직한 일반화 검증)*
...
현재 구현은 **하이브리드 2-모드**다 (상단 [★ 최신 업데이트 C](#-최신-업데이트--v3-신호등-분리--한국어-인식--범용-입력-2026-05-31) 참조). 입력 종류에 따라 클라이언트가 자동으로 모드를 선택한다.
