...
**문제** — 검출·추적·인식 파이프라인을 엣지에서 실시간 구동하려면 양자화가 필수다. 그런데 *어느 단계를, 어떤 정밀도로, 어떤 런타임에서* 압축해야 하는지는 자명하지 않다.

**논지** — 양자화 민감도는 흔히 믿는 *기법*(W8A8냐 SmoothQuant냐)이 아니라 **모델 아키텍처와 배포 런타임에 의존한다.** 본 프로젝트는 깨끗한 분류 백본(대조군)에서 실제 검출 파이프라인까지 동일한 양자화를 적용해 이 의존성을 실측으로 드러낸다.

**반직관 핵심 발견 3가지** (실측으로만 얻어지는, 본 연구의 진짜 기여):

1. **검출 헤드(DFL)가 INT8의 진짜 벽이다.** 백본에서 무손실인 INT8이 YOLO 검출 헤드까지 적용되면 출력 CosSim이 **0.9995여도 검출이 0으로 붕괴**한다. → 헤드는 FP32로 남겨야 하고, **CosSim 같은 텐서 유사도는 양자화 검증에 무용** — 실프레임 검출 수·conf로 봐야 한다. (이 제약이 Phase 12에서 YOLO26(DFL 제거)으로 가는 동기다.)
2. **"저정밀 = 고속"은 런타임에 따라 거짓이다.** 브라우저 온디바이스에서 **FP32/WebGPU 62 FPS**가 INT8/WASM(2.2 FPS)·FP16/WebGPU(24 FPS)보다 빠르다. INT8 가속은 *서버 CPU에서만* 정답(2.4×). 정밀도 선택은 성능표가 아니라 **타깃 런타임**이 결정한다.
3. **단계별 민감도는 인식기 > 검출기지만, '예상된' 인식기보다 검출 헤드가 더 단단한 벽이다.** 검출기 W8A8은 mAP −0.07%p(무손실), 인식기 OCR은 W4A16에서 **−43.9%p 붕괴**. 민감도 순위 자체는 예상대로지만(발견 ③), 진짜 배포 난점은 발견 ①의 *헤드*였다.
...
**양자화 민감도가 기법이 아니라 아키텍처·런타임에 의존함**을 깨끗한 분류 백본(대조군)부터 실제 검출 파이프라인까지
동일하게 측정하고, 실제 도로 도메인 실시간 시연 시스템까지 세 단계로 진행되었다.

| 단계 | 주제 | 핵심 결과 |
| :--- | :--- | :--- |
| **Phase 1** | **대조군**: 깨끗한 분류 백본 | 6개 양자화 기법 스크리닝 → 백본에선 모두 *우아하게* degrade, SmoothQuant 승(Final Score **0.8068**). **§6 파이프라인과 대조되는 기준선** — 여기선 안 망가지는 INT8이 검출 헤드에선 붕괴한다 |
...
**결론:** 단계별 민감도 순위는 **인식기(W4A16/1-Bit) > 검출기(W4A16) > 검출기(W8A8) ≈ 무손실**로, *예상대로* 인식기가 가장 민감했다(OCR W4A16 −43.9%p 붕괴). 검출기 W8A8은 mAP −0.07%p·MOTA −1.4%로 무손실, W4A16은 mAP −11.0%p·MOTA −40.3%로 저하.

그러나 **순위보다 중요한 두 가지 반직관 발견**이 실측에서 드러났다:
- **양자화 단위(layer)가 단계(stage)보다 결정적이다.** 검출기를 "통째로" W8A8화하면 무손실이지만, 그 안의 **검출 헤드(DFL)** 까지 INT8화하면 CosSim 0.9995에도 검출이 0으로 붕괴한다 — 민감도의 진짜 경계는 단계가 아니라 *헤드 vs 백본*이다.
- **실시간성의 레버는 정밀도가 아니라 런타임이다.** "낮은 비트 = 빠름"은 서버 CPU에서만 성립(INT8 2.4×)하고, 브라우저에선 FP32/WebGPU가 INT8·FP16을 모두 앞선다.

주야간 stratified test 시퀀스의 추적 MOTA는 E0 0.295 → E5 0.280(−5.1%)로 유지된다(절대값 0.295는 난조건 기준 — 본 연구의 변수는 *상대 열화*다).
...
> **Phase 1의 역할 — 대조군(control).** Phase 1은 배포 모델을 만들거나 "여기서 이긴 기법을 그대로 이식"하는 단계가 *아니다*. 빠르게 반복 가능한 ConvNeXtV2-Nano 분류 백본에서 6개 기법을 스크리닝해, **깨끗한 단일 분류기에서 양자화가 어떻게 거동하는지의 기준선**을 세운다.
> 핵심은 *대조*다: 백본에선 모든 기법이 우아하게 degrade하고 SmoothQuant가 최선이지만(아래 표), **이 우아함은 검출 파이프라인으로 전이되지 않는다** — [§6](#6-phase-2--양자화-실험-매트릭스)에서 같은 INT8이 검출 헤드를 붕괴시킨다. "ConvNeXt에서 좋은 기법이 YOLO에서도 좋은가?"의 답은 *부분적으로만 예*이며, **그 불일치 자체가 본 연구의 핵심 발견**이다.
> (Phase 1의 Perf 지표는 멀티모달 검색 프록시이므로 점수를 그대로 이식하지 않고, Phase 2에서 검출 mAP·OCR Top-1 등 태스크 고유 지표로 독립 재측정한다.)
...
**W8A8 SmoothQuant를 Phase 2의 *출발 기법*으로 가져가되, 전이 여부는 §6에서 태스크 고유 지표로 독립 검증한다.** 결과를 미리 말하면: 검출기 W8A8/SmoothQuant는 무손실로 전이되지만(§6.2), 같은 INT8을 **검출 헤드**에 넣거나 인식기를 **W4A16**으로 내리면 전이되지 않고 붕괴한다(§6.3, §8) — 대조군의 우아한 거동이 파이프라인에서 깨지는 지점이다.
...
> **Q&A의 위치 — 프레젠테이션 레이어(연구 기여 아님).** 주행 Q&A는 구조화된 인식 결과(JSON)를 자연어로 보여주는 *데모 UX*이며, 양자화·엣지 추론이라는 본 연구의 기여와는 무관하다. 템플릿으로도 답할 수 있는 기능을 LLM으로 대체한 것에 가깝고, 의도적으로 BYOK(사용자 키)·선택 기능으로 두었다. 평가는 §1–§8의 양자화·배포 결과를 기준으로 해주기 바란다.
