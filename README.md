...
| `POST /api/qa` | 200 (Groq API 키 설정 후 SSE 스트리밍 응답) |

Phase 7 Q&A 흐름은 Groq API(무료 티어, Llama 3.3 70B 기본)로 동작한다. `.env`에 `GROQ_API_KEY` 설정 후 다음 명령으로 실행한다. API 키는 https://console.groq.com/keys 에서 Google 계정으로 로그인하여 무료 발급한다.
