...
<!--
  데모 영상/사진 첨부 위치 — 아래 자리표시자를 실제 미디어로 교체하면 된다.

  • GIF / 스크린샷:   ![Web Demo](assets/demo/web_demo.gif)
  • GitHub 업로드 영상: 이슈/PR 편집창에 mp4를 드래그하면 생성되는
                        https://github.com/<user>/<repo>/assets/.../demo.mp4 링크를 그대로 붙여넣으면
                        README에서 자동으로 재생 플레이어로 임베드된다.
-->

| 검출 · 추적 · 인식 데모 | 주행 Q&A 데모 |
| :---: | :---: |
| _웹 데모 영상 첨부 예정_ | _Q&A 데모 영상 첨부 예정_ |

> 위 표의 자리표시자를 실제 데모 영상/스크린샷으로 교체할 예정이다.
> (현재 단일 프레임 정성 결과는 [7장 도메인 적응](#7-phase-3--도메인-적응-신호등-분리-검출-및-한국어-인식)의 청량리역 사거리 추론 샘플에서 확인할 수 있다.)
...
- [부록 A. 옴니모달(VLM) 탐색적 실험 (Phase 1 곁가지)](#부록-a-옴니모달vlm-탐색적-실험-phase-1-branch)
