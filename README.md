...
1. **검출 헤드(DFL)가 INT8의 진짜 벽.** backbone에서 무손실인 INT8이 YOLO 검출 헤드까지 적용되면 출력 CosSim이 **0.9995여도 검출이 0으로 붕괴**. → 헤드는 FP32로 남겨야 하고, **CosSim 같은 텐서 유사도는 양자화 검증에 무용** — 실프레임 검출 수·conf로 검증. (이 제약이 Phase 12에서 YOLO26(DFL 제거)으로 가는 동기.)
...
> **향후 확장 (Future Work — 본 저장소의 보고 결과에는 미포함):** 검출기를 **YOLO26**(NMS-free · DFL 제거)으로 교체하면 위 "검출 헤드 INT8 붕괴" 제약을 구조적으로 없애 헤드까지 풀 INT8 양자화 가능. **폰 온디바이스(WebGPU)** 에서 정밀도 사다리(FP32→FP16→INT8→W4A16)로 시연하는 확장을 별도로 진행 중. 현재 README의 모든 정량 결과·시연은 **v3(YOLOv8s) 파이프라인** 기준.
...
| 검출 헤드 INT8 | 활성화(가중치 아님) | 헤드 INT8 SQNR 34 ≥ backbone 33 dB · 초과첨도 49 · DFL 0.0015 bin | 헤드 QDQ 제외 / 구조적으론 DFL 제거(→ YOLO26) |
| 검증 함정 | CosSim의 맹점 | CosSim ≈ 1.000 · 검출 유지율 0% | 텐서 유사도 금지, 실프레임 검출수·conf로 검증 |

> 이 분석이 [Future Work](#82-프로덕션-고도화-및-향후-확장)의 YOLO26(DFL·NMS 제거) 방향에 **근거**를 제공: 헤드 붕괴는 가중치가 아니라 DFL 구조가 만든 활성화-측·임계값 취약성이므로, DFL을 구조적으로 없애면 헤드까지 INT8 양자화 가능성이 열림.
