...
└──────────────────────────┘
       ...
       ▼  [선택: Q&A 모드]
┌──────────────────────────┐    ┌──────────────────┐
│ 5. Q&A 브리지 레이어     │───►│  Groq LLM API     │
│  build_context(tracks)   │    │  (Llama 3.3 70B,  │
│  → 자연어 컨텍스트 문자열 │◄───│   무료·고속)      │
│  사용자 질문 + 컨텍스트   │    │  스트리밍 답변    │
│  → Groq에 전달           │    └──────────────────┘
└──────────────────────────┘
  (엣지 파이프라인 결과가 클라우드 LLM과 결합)
```

**Q&A 레이어 컨텍스트 예시:**
```
인식된 객체 (프레임 #1234):
  [Track #2] 교통표지판 - '속도제한50' (신뢰도 94%)
  [Track #5] 간판 - '카페베이커리' (신뢰도 87%)

사용자: "저 표지판이 뭘 의미하는 거야?"
Groq LLM: "속도제한 50km/h 구간입니다. 해당 구간에서는 시속 50km 이하로 주행해야 합니다..."
```
...
### 모드 1: 서버 어시스트 + Q&A (주행 데모 — 우선 구현)

```
브라우저                    FastAPI 서버 (src/pipeline/app.py)
┌──────────┐  WebSocket  ┌──────────────────────┐
│ Camera/  │ ──────────► │ YOLOv8n ONNX          │
│ 동영상    │             │ + ByteTrack            │
│ 프레임    │ ◄────────── │ + OCR/분류 ONNX       │
│ 전송     │  JSON 결과  │                        │
│          │             └──────────────────────┘
│ 질문 입력 │  POST /api/qa (SSE)
│ 답변 표시 │ ──────────► qa_bridge.py → Groq LLM API
└──────────┘ ◄────────── 스트리밍 답변
```
...
| 2026-05-28 | 주행 Q&A: Cloud API 하이브리드 방식 (엣지 파이프라인 + Claude Haiku) | 엣지 제약(<15MB)으로 온디바이스 LLM 불가 (SmolLM2-135M W1도 ~17-67MB). 연구 결론부 데모에서는 "엣지 인식 + 클라우드 지능"이 가치 명확. Claude Haiku 사용으로 저비용·빠른 응답. | 온디바이스 소형 LLM(제약 초과), 규칙 기반 답변(유연성 부족) |
| 2026-05-30 | Q&A LLM을 Claude Haiku → Groq Llama 3.3 70B 로 교체 | Groq 무료 티어로 비용 0, LPU 추론 속도가 Haiku보다 빠름(체감 즉답), OpenAI 호환 API라 `qa_bridge.py` 교체 비용 최소. 70B 모델이라 한국어 품질도 충분. | Gemini Flash(분당 RPM 더 낮음), OpenRouter 무료(모델 가용성 불안정), Ollama 로컬(데모 환경 제약) |
