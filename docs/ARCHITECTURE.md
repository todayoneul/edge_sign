...
└──────────────────────────┘
       ...
       ▼  [선택: Q&A 모드]
┌──────────────────────────┐    ┌──────────────────┐
│ 5. Q&A 브리지 레이어     │───►│  Claude API       │
│  build_context(tracks)   │    │  (Haiku, 저비용)  │
│  → 자연어 컨텍스트 문자열 │◄───│  스트리밍 답변    │
│  사용자 질문 + 컨텍스트   │    └──────────────────┘
│  → Claude에 전달         │
└──────────────────────────┘
...
Claude: "속도제한 50km/h 구간입니다. 해당 구간에서는 시속 50km 이하로 주행해야 합니다..."
```
...
│ 답변 표시 │ ──────────► qa_bridge.py → Claude Haiku API
└──────────┘ ◄────────── 스트리밍 답변
```
