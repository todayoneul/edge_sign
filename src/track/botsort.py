...
def preprocess_crop(img_bgr: np.ndarray, bbox: np.ndarray, size: int = 64) -> np.ndarray:
...
class CMC:
    ...
    def apply(self, frame_bgr: np.ndarray) -> np.ndarray:
        ...
        if (
            self.prev_des is not None
            and des is not None
            and len(kp) >= 4
            and len(self.prev_kp) >= 4
        ):
            matches = self.bf.knnMatch(self.prev_des, des, k=2)
            good = [m for m, n in matches if m.distance < self.ratio * n.distance]

            if len(good) >= 4:
                src_pts = np.float32([self.prev_kp[m.queryIdx].pt for m in good]).reshape(-1, 1, 2)
...
class BoTSORTTracker:
    ...
    def _matching_cost(
        ...
        if det_feats is not None and any(t.smooth_feat is not None for t in tracks):
            # ReID 코사인 거리
            t_feats = np.array(
                [
                    t.smooth_feat if t.smooth_feat is not None else np.zeros(det_feats.shape[1])
                    for t in tracks
                ]
            )
    ...
    def update(self, detections: np.ndarray, frame_bgr: np.ndarray | None = None) -> list[BoTTrack]:
        ...
        if len(confirmed) and len(dets_high):
            ...
            matches1, unmatched_t, unmatched_d = linear_assignment(cost, thresh=self.match_thresh)
        else:
            matches1 = []
        ...
        if len(unmatched_tracks_1) and len(dets_low):
            tlwh_low = np.array([[d[0], d[1], d[2] - d[0], d[3] - d[1]] for d in dets_low])
            iou_d2 = iou_distance(np.array([t.tlwh for t in unmatched_tracks_1]), tlwh_low)
        ...
        if len(self.lost_stracks) and len(unmatched_d):
            ...
            feats_ud = (
                det_feats_high[unmatched_d]
                if det_feats_high is not None and len(unmatched_d)
                else None
            )
            tlwh_ud = (
                np.array([[d[0], d[1], d[2] - d[0], d[3] - d[1]] for d in det_arr_ud])
                if len(det_arr_ud)
                else np.zeros((0, 4))
            )
        ...
        if len(unconfirmed) and len(unmatched_d_new):
            ...
            tlwh2 = (
                np.array([[d[0], d[1], d[2] - d[0], d[3] - d[1]] for d in det_arr2])
                if len(det_arr2)
                else np.zeros((0, 4))
            )
        ...
        self.lost_stracks = [
            t
            for t in _sub(self.lost_stracks, self.tracked_stracks)
...
class OnnxReIDNet:
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        out = self.sess.run([self.output_name], {self.input_name: x.numpy()})[0]
...
if __name__ == "__main__":
    ...
    dets = np.array([[100, 200, 200, 350, 0.9, 0], [300, 100, 450, 280, 0.8, 1]], dtype=np.float32)
