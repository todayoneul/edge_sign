...
def main():
    ...
    print(
        f"Dataset loaded. Train samples: {len(train_dataset)}, Validation samples: {len(val_dataset)}"
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            f"Epoch [{epoch}/{EPOCHS}] - Train Loss: {epoch_loss:.4f}, Train Acc: {train_acc:.2f}% | Val Loss: {epoch_val_loss:.4f}, Val Acc: {val_acc:.2f}%"
        )
