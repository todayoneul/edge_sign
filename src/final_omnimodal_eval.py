...
def main():
    ...
    for config in MODELS_TO_EVAL:
        ...
        print(
            f"Recall@1: {recall_1:.2f}% | Latency: {latency * 1000:.2f}ms | Throughput: {throughput:.2f}fps | Memory: {config['memory_mb']}MB"
        )
    ...
    print(
        f"{'Model':<25} | {'Recall@1':<10} | {'Latency':<10} | {'Memory':<10} | {'Final Score':<10}"
    )
    print("-" * 75)
    for r in results:
        print(
            f"{r['name']:<25} | {r['recall']:<8.2f}% | {r['latency'] * 1000:<8.2f}ms | {r['memory']:<8.2f}MB | {r['final_score']:<10.4f}"
        )
    ...
    with open("logs/final_score_report.txt", "w", encoding="utf-8") as f:
        ...
        for r in results:
            f.write(
                f"{r['name']},{r['recall']:.2f},{r['latency'] * 1000:.2f},{r['memory']:.2f},{r['perf_norm']:.4f},{r['speed_norm']:.4f},{r['mem_norm']:.4f},{r['final_score']:.4f}\n"
            )
