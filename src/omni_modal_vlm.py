...
class BinaryConv2d(nn.Conv2d):
    def forward(self, input):
        ...
        return torch.nn.functional.conv2d(
            input, bw, bias, self.stride, self.padding, self.dilation, self.groups
        )
...
def build_llm_and_tokenizer(
    llm_name, use_qlora=False, lora_r=16, lora_alpha=32, lora_dropout=0.05, lora_target_modules=None
):
    if use_qlora:
        try:
