...
def print_results(results: list):
    ...
    print(
        f"{'E0':<6} {'0.6275':<10} {'0.4371':<14} {'0.722':<8} {'0.543':<8} {e0_size:<10.2f}  ← FP32 기준선"
    )

    for exp_id, onnx_path, m in results:
        size_mb = onnx_path.stat().st_size / 1024 / 1024
        print(
            f"{exp_id:<6} {m['map50']:<10} {m['map']:<14} {m['precision']:<8} {m['recall']:<8} {size_mb:<10.2f}"
        )
...
def main():
    ...
    parser.add_argument(
        "--exp", choices=list(EXPERIMENT_MAP.keys()) + ["all"], default="all", help="실행할 실험 ID"
    )
