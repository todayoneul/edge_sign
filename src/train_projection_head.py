...
def train_projection_head(args):
    ...
    if args.vision_ckpt and os.path.exists(args.vision_ckpt):
        ...
        print(
            "경고: Vision Encoder 체크포인트가 지정되지 않았거나 파일을 찾을 수 없습니다. 랜덤 초기화로 진행합니다."
        )
    ...
    optimizer = torch.optim.AdamW(
        model.projection_head.parameters(), lr=args.lr, weight_decay=args.weight_decay
    )
    ...
    for epoch in range(args.epochs):
        ...
        progress_bar = tqdm(
            ...
            total=estimated_total_samples // args.batch_size,
        )
        ...
        for batch in progress_bar:
            ...
            if steps % 5000 == 0:
                save_path = os.path.join(
                    args.save_dir, f"projection_head_epoch_{epoch + 1}_step_{steps}.pth"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--vision_ckpt",
        type=str,
        default="./models/hf_w8a8_smoothquant/smoothquant_w8a8.pth",
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default="./checkpoints/vlm_projection",
        help="Directory to save checkpoints",
    )
    parser.add_argument(
        "--max_samples", type=int, default=None, help="Max samples for training (for quick testing)"
    )
    parser.add_argument(
        "--sources", type=str, default=None, help="쉼표로 구분된 데이터셋 이름 목록"
    )
    parser.add_argument(
        "--source_weights", type=str, default=None, help="쉼표로 구분된 데이터셋 가중치 목록"
    )
    parser.add_argument(
        "--cache_dir", type=str, default="./data/vlm_cache", help="이미지 캐시 디렉터리"
    )
