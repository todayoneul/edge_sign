...
CHECKPOINT_PATH = (
    "./checkpoints/checkpoints_mm_fp16/mm_fp16_epoch_15.pth"  # Needs to be trained first
)
    ...
    with torch.no_grad():
        for i, (images, _) in enumerate(dataloader):
            if i >= num_calib_batches:
                break  # Calibrate on configurable batches
...
def main():
    ...
    parser.add_argument(
        "--calib-batches", type=int, default=5, help="Number of calibration batches (default: 5)"
    )
    parser.add_argument(
        "--alpha", type=float, default=0.5, help="SmoothQuant alpha ratio (default: 0.5)"
    )
    ...
    model_sq = calibrate_and_apply_smoothquant(
        model, calib_loader, alpha=args.alpha, num_calib_batches=args.calib_batches
    )
