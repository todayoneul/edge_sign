...
EPOCHS = 15
LEARNING_RATE = 5e-5
SAVE_DIR = "./checkpoints/checkpoints_mm_w8a8"
LOG_DIR = "./logs"
...
def fake_quantize_8bit(weight):
    ...
    q_weight = torch.clamp(RoundSTE.apply(weight / scale), -128, 127)
    return q_weight * scale


class W8A8Conv2d(nn.Conv2d):
    def forward(self, input):
        w_q = fake_quantize_8bit(self.weight).to(input.dtype)
        bias = self.bias.to(input.dtype) if self.bias is not None else None
        return F.conv2d(input, w_q, bias, self.stride, self.padding, self.dilation, self.groups)


class W8A8Linear(nn.Linear):
    def forward(self, input):
        w_q = fake_quantize_8bit(self.weight).to(input.dtype)
        bias = self.bias.to(input.dtype) if self.bias is not None else None
...
def replace_layers_with_w8a8(model):
    for name, module in model.named_children():
        if isinstance(module, nn.Conv2d) and "head" not in name:
            ...
            replace_layers_with_w8a8(module)
...
def main():
    print("Starting W8A8 Multimodal QAT...")
    csv_file_path = os.path.join(LOG_DIR, "training_log_mm_w8a8.csv")
    ...
    print("Loading W8A8 Student Model...")
    student_model = timm.create_model(MODEL_NAME, pretrained=True)
    replace_layers_with_w8a8(student_model)
    ...
    for ckpt_file in glob.glob(os.path.join(SAVE_DIR, "mm_w8a8_epoch_*.pth")):
        try:
    ...
    if latest_epoch > 0:
        resume_checkpoint = os.path.join(SAVE_DIR, f"mm_w8a8_epoch_{latest_epoch}.pth")
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        for i, (s_images, t_images) in enumerate(train_loader):
            s_images, t_images = (
                s_images.to(DEVICE, dtype=torch.bfloat16),
                t_images.to(DEVICE, dtype=torch.bfloat16),
            )
            ...
            loss = criterion(
                student_features, teacher_features, torch.ones(s_images.size(0), device=DEVICE)
            )
            loss.backward()
            optimizer.step()
        ...
        torch.save(
            ...
            os.path.join(SAVE_DIR, f"mm_w8a8_epoch_{epoch}.pth"),
        )
