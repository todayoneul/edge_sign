...
def main():
    ...
    if not train_dir.exists() or not val_dir.exists():
        print(
            f"Error: Dataset directories not found at {DATA_DIR}. Run prepare_korean_ocr_data.py first."
        )
        return
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            f"  Train Loss: {epoch_loss:.4f} | Top-1 Acc: {train_acc1:.2f}%, Top-5 Acc: {train_acc5:.2f}%"
        )
        print(
            f"  Val Loss:   {epoch_val_loss:.4f} | Top-1 Acc: {val_acc1:.2f}%, Top-5 Acc: {val_acc5:.2f}%"
        )
