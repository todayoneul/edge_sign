...
plt.title(
    "Omni-Modal Alignment Progress (Validation Cosine Similarity)", fontsize=14, fontweight="bold"
)
...
if os.path.exists("logs/final_score_report.txt"):
    ...
    # Scatter plot
    sns.scatterplot(
        data=df_score, x="Memory(MB)", y="FinalScore", hue="Model", s=200, palette="deep"
    )
    ...
    plt.xscale("log")
    plt.title(
        "Omni-Modal Models: Memory vs Final Score (Pareto Frontier)", fontsize=14, fontweight="bold"
    )
