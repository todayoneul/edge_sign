...
**배포 형태.** 시스템은 두 가지 방식으로 실행된다. 서버 방식은 브라우저가 프레임을 FastAPI 서버로 보내고 좌표를 받아 그린다. 온디바이스 방식은 검출·추적·인식 전체를 ONNX Runtime Web으로 브라우저 안에서 실행한다. 두 방식은 같은 결과 형식과 렌더링 코드를 공유하므로 사용자는 화면에서 실행 위치만 바꿔 비교할 수 있다. 본 논문의 측정은 온디바이스 방식의 추론 경로를 대상으로 한다.
