...
신경망 양자화는 모델 크기와 연산량을 줄이는 대표적 방법이다 [5], [6]. 그러나 파이프라인에 양자화를 적용할 때는 두 가지를 따로 따져야 한다. 첫째, 양자화 오차가 어느 구성요소에서 태스크 정확도로 이어지는가이다. 검출기의 몸통과 헤드, 뒤 단계의 인식기는 구조와 출력 형식이 달라서 같은 설정에서도 손실이 다를 수 있다 [7]–[10]. 특히 YOLO 계열에서는 박스 좌표와 클래스 점수를 한 출력으로 연결하는 헤드가 두 값의 동적 범위 불일치 때문에 INT8 변환에서 검출을 모두 잃는다는 사실이 모바일 TFLite 변환에서 보고되었고, 좌표를 정규화하는 완화책도 제안되었다 [10]. 그러나 이 실패가 브라우저가 쓰는 ONNX 그래프와 실행 환경에서 어떻게 나타나며 파이프라인의 정확도와 지연에 어떤 결과를 낳는지는 확인되지 않았다. 둘째, 양자화 모델이 목표 실행 환경에서 실제로 빨라지는가이다. INT8의 속도 이득은 연산 장치와 커널 지원에 따라 달라지며 [11], 브라우저에서는 이런 차이가 더 클 수 있다 [1], [2]. 기존 검출기 양자화 연구는 대부분 단일 모델의 정확도나 한 가지 하드웨어에서의 지연을 다루므로, 구성요소와 실행 환경을 함께 바꿨을 때의 결과는 알기 어렵다.
...
박스 좌표와 클래스 점수의 혼합 범위 실패 자체는 선행 연구 [10]가 먼저 보고하였다. 본 논문은 이 알려진 실패와 실행 환경의 효과를 브라우저 배포 관점에서 분리하고 연결하며, 기여는 다음과 같다.
1. **알려진 혼합 범위 실패의 재현 (4.2절, 4.5절).** 모바일 TFLite에서 보고된 실패 [10]가 YOLOv8s, YOLO26-n, YOLO11l의 ONNX QDQ 그래프에서도 재현됨을 보인다. YOLO11l에서는 측정 전에 공개한 예측으로 확인한다.
2. **텐서 수준의 원인 분리 (4.2절).** 가중치 전용·활성값 전용·단일 텐서 ablation으로 실패를 일으키는 텐서와 그 필요·충분 조건을 분리하고, 단일 척도가 좌표와 점수를 함께 보존할 수 없는 조건을 식으로 정리한다.
3. **실패 회피 뒤 남는 손실과 종단 전파 (4.2절).** 헤드·디코드 단계를 FP32로 두거나 알려진 좌표 정규화를 적용한 뒤 남는 정확도 손실을 분해하고, 그 손실이 검출–추적–인식 종단 정확도로 커져 전파되는 크기를 측정한다.
4. **브라우저 실행 환경의 연산자 배치와 지연 (4.3–4.4절).** WASM과 WebGPU, ONNX Runtime Web 버전에 따른 연산자 배치와 지연을 분석하고, 구성요소마다 실행 환경을 나눈 배치를 평가한다.
5. **기기 간 비교 (4.3–4.4절).** Windows/NVIDIA와 Mac/Apple에서 같은 절차로 측정해, 두 기기에서 같았던 결과와 기기에 따라 달라진 결과를 구분한다.
...
정수 양자화는 가중치와 활성값을 낮은 비트의 정수로 나타내 저장과 연산 비용을 줄인다 [5]. 채널별 대칭 가중치와 비대칭 활성값, 보정 데이터 기반 범위 추정은 학습 후 양자화(PTQ)의 표준 구성이다 [6]. 검출기에 대해서는 전체 그래프의 저비트 변환 [7], YOLO의 PTQ [8], 박스 회귀 분기에 특화된 보정 [9]이 제안되었다. Niu 등 [18]은 텐서 오차 대신 태스크 손실로 보정 지표를 정해야 한다고 보았다. 이 연구들은 대부분 새로운 양자화 방법을 제안하고 단일 모델의 정확도나 한 가지 하드웨어의 지연으로 평가한다.

본 연구와 가장 가까운 선행 연구는 Moon 등 [10]이다.
- **보고한 실패:** YOLOv6의 분리형 헤드는 [0, W) 범위의 박스 좌표와 [0, 1] 범위의 클래스 점수를 하나의 출력으로 연결한다. 이들은 두 값의 동적 범위 불일치 때문에 TFLite INT8 변환에서 mAP가 0%가 됨을 보였다.
- **완화책:** YOLOv8은 박스 좌표를 [0, 1]로 정규화한 뒤 후처리에서 입력 해상도로 되돌리는 방식(vanilla rescaling)으로 이 문제를 완화하였다. Moon 등은 이 방식에서도 좌표에 stride를 곱하는 단계의 양자화 오차가 크다고 분석하고, 앵커와 회귀 출력을 stride/W로 각각 정규화한 뒤 더하는 Regression Normalization(RN)을 제안하였다. RN은 COCO에서 YOLOv6n과 YOLOv8n의 TFLite INT8 mAP를 높였다(YOLOv8n 28.4% → 31.9%, FP16 35.3%).
- **범위:** 실험은 TFLite 2.16의 정수 PTQ와 모바일 기기(Galaxy S23)의 추론 속도로 이루어졌다. 현행 Ultralytics(8.4)의 TensorFlow·TFLite 내보내기도 앵커와 거리를 입력 크기로 먼저 정규화한다 [16].

본 연구는 이 실패를 새로 발견했다고 주장하지 않으며, 새 양자화 알고리즘도 제안하지 않는다. Moon 등과 다른 점은 다음과 같다.
- **그래프와 런타임:** TFLite 대신 브라우저가 쓰는 ONNX Runtime의 정적 QDQ 그래프를 평가하고, NMS가 필요 없는 YOLO26-n을 포함한 세 검출기에서 실패가 어느 텐서까지 퍼지는지 확인한다.
- **원인 분리:** 층별 양자화 오차 대신 가중치 전용·활성값 전용·단일 텐서 양자화로 실패를 일으키는 텐서와 그 필요·충분 조건을 확인한다.
- **남는 손실과 전파:** 실패를 피하는 여러 방법(헤드·디코드 단계 FP32, 좌표 정규화) 뒤에 남는 손실과, 그 손실이 파이프라인의 종단 정확도로 전파되는 크기를 측정한다.
- **실행 환경:** 정확도 문제를 피한 모델이 브라우저 WASM·WebGPU에서 실제로 빨라지는지를 연산자 배치와 두 기기의 지연으로 확인한다.
...
**Table 1. 관련 연구와의 범위 비교.** ✓는 해당 항목을 주 실험에서 다룬 경우이다. "태스크 정확도"는 텐서 오차가 아닌 태스크 지표(mAP, Top-1, 추적 지표)로 평가한 경우, "연산자 배치"는 지연 차이를 연산자가 실행된 장치 수준에서 분석한 경우이다. "혼합 범위 실패"는 박스 좌표와 클래스 점수가 한 양자화 범위를 공유해 생기는 검출기 헤드의 실패를 다룬 방식이다.

| 연구 | 양자화 | 브라우저 | WebGPU | 다중 구성요소 | 태스크 정확도 | 연산자 배치 | 혼합 범위 실패 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| Ma 등 [1] | – | ✓ | – | – | – | – | – |
| Wang 등 [2] | – | ✓ | – | – | ✓ | – | – |
| Lee와 Jeon [19] | – | ✓ | – | – | – | – | – |
...
**Fig. 2. 구성요소·정밀도별 정확도 유지율.** 검출기는 mAP@0.5:0.95, 인식기는 Top-1이며, 괄호는 파일 크기이다. 수염은 95% 프레임 부트스트랩 신뢰구간이고, 파선은 99% 기준, 점선은 95% 참고 등급이다. 전체 INT8 검출기는 검출이 하나도 없어 0%이다. FP32 bias 변형은 INT32 bias 변형과의 차이가 0.1%p 이내라 생략하였다.

**붕괴 원인: 가중치·활성값·단일 텐서 분리.** Moon 등 [10]은 YOLOv6/TFLite에서 이 붕괴를 박스 좌표와 클래스 점수의 동적 범위 불일치로 설명하였고, 부록 A의 선행 탐색에서도 붕괴 원인이 가중치가 아니라 활성값 양자화에 있다는 가설을 세웠다. 이 설명이 ONNX QDQ 그래프에서도 성립하는지, 그리고 어느 텐서의 양자화가 필요하고 충분한지를 3.2절의 원인 분리 변형으로 확인하였다(Table 5).
...
**메커니즘.** Moon 등 [10]이 보고한 기제와 같이, 점수와 박스 좌표가 섞인 텐서의 범위는 박스 좌표가 정한다.
...
**단일 척도의 한계.** 식 (2)–(4)의 단일 UINT8 척도와 최근접 반올림에서, 범위에 0을 포함하는 텐서의 값 $x$는 영점 $z$에서 $\lfloor x/s\rceil$만큼 떨어진 정수로 표현된다. 좌표와 점수를 함께 담는 텐서에는 다음 두 조건이 동시에 걸린다(반올림 경계는 제외).

$$\underbrace{s\ \ge\ \frac{640}{255}\approx 2.51}_{\text{0–640 좌표를 잘림 없이 표현}}\qquad\qquad \underbrace{\frac{c}{s}\ \ge\ \frac{1}{2}\ \ \Leftrightarrow\ \ s\ \le\ 2c}_{\text{점수 }c\text{가 영점과 다른 정수로 남음}} \tag{6}$$

- 점수 1.0조차 $s\le 2$를 요구하므로, 좌표 범위 전체를 담는 척도($s\ge 2.51$)에서는 모든 점수가 영점으로 반올림된다. 붕괴 텐서의 실측 척도 2.51–3.72가 모두 이 경우이다.
- 운영 임계값 부근의 점수 $c=0.25$를 보존하려면 $s\le 0.5$, 즉 범위가 127.5 이하여야 하며, 그러면 이를 넘는 좌표는 잘린다.
- 따라서 하나의 척도로는 전체 좌표 범위와 낮은 신뢰도의 점수를 동시에 보존할 수 없다.
- 이 결론은 관측한 최솟값·최댓값을 범위로 쓰는 MinMax 보정에 대한 것이다. Percentile·Entropy처럼 범위의 일부를 잘라내는 보정은 좌표 잘림과 점수 보존 사이의 절충점을 옮기는 방법이며, 그 효과는 이 절 끝의 보정 실험으로 확인한다.
...
$$d=\sum_{i=0}^{15} w_i\,p_i,\qquad w_i=i,\qquad \sum_{i} p_i=1 \tag{7}$$
...
**알려진 완화책: 박스 좌표 정규화.** Moon 등 [10]과 Ultralytics의 TensorFlow·TFLite 내보내기가 쓰는 좌표 정규화를 같은 ONNX 그래프에 적용하였다(설계와 예측은 측정 전에 작성해 결과와 함께 공개하였다. COCO 검증과 달리 측정 전 커밋으로 고정하지는 않았다).
- **변형:** (i) 픽셀 좌표를 계산한 뒤 1/640을 곱하는 사후 정규화(vanilla rescaling), (ii) 거리와 앵커를 stride/640으로 먼저 정규화하는 사전 정규화(현행 Ultralytics 방식), (iii) stride/640을 회귀 합성곱에 접어 넣는 방식(RN과 같은 구조, DFL이 없는 YOLO26-n만 가능)이다. 640 배율 복원은 세 경우 모두 그래프 밖의 후처리에서 하였다. Moon 등의 그림은 복원 곱셈을 출력 연결 뒤에 두므로, 이 구현을 RN이라 부르지는 않는다.
- **결과:** 정규화한 FP32 그래프는 원래 FP32와 같은 mAP를 냈다. 전체 INT8로 양자화하면 정규화된 연결 텐서의 척도가 0.004–0.006으로 식 (6)의 조건을 만족해 점수가 보존되었고, 붕괴는 사라졌다(유지율 34–55%, Table 5).
- **한계:** 그러나 유지율은 디코드 단계만 FP32로 둔 경우(92.8–94.3%)나 헤드 전체를 FP32로 둔 경우(97.0–98.7%)에 크게 못 미쳤다. [0, 1]로 정규화한 좌표도 255단계로 반올림되어 640 입력에서 약 2.5 px 간격이 남고, 사후 정규화에서는 그 앞의 픽셀 단위 텐서(척도 2.51–3.72)도 양자화되기 때문이다. 이 손실은 작은 객체에 몰렸다(입력 기준 한 변 8 px 미만 6–16%, 16 px 이상 65–71%).

**좌표 반올림과 객체 크기.** 좌표 양자화의 손실이 객체 크기에 따라 다른지를, 측정 전에 정한 규칙으로 확인하였다(Fig. 3). 규칙은 좌표만 반올림한 세 조건에서 가장 큰 구간과 가장 작은 구간의 유지율 차이의 95% 신뢰구간이 모두 0보다 크면 지지하는 것이다.
- 가중치를 FP32로 두고 붕괴 텐서를 뺀 디코드 단계 활성값만 양자화하면(점수는 보존, 좌표만 반올림), 작은 객체의 유지율은 YOLOv8s 21%, YOLO26-n 11%로 큰 객체(74%, 72%)보다 크게 낮았다. COCO의 YOLO11l에서도 같았다(small 70%, large 94%). 세 조건 모두 규칙을 만족해 가설이 지지되었다.
- 반면 디코드 단계를 FP32로 둔 헤드 제외·디코드 제외 INT8에서는 크기에 따른 차이가 없었다(차이의 신뢰구간이 모두 0을 포함).
- 도로 표지는 640 입력 기준 한 변의 중앙값이 11.6 px로 작다. 붕괴 텐서를 뺀 좌표 양자화의 손실이 COCO(86.7%)보다 도로 검출기(57.0–61.2%)에서 큰 것도 이 크기 차이와 부합한다. 다만 두 워크로드는 모델도 다르다.

![Fig. 3](figures/fig13_size_retention.png)

**Fig. 3. 객체 크기별 유지율.** (a)(b) 도로 test의 두 검출기(640 입력 기준 한 변 8 px 미만, 8–16 px, 16 px 이상), (c) COCO의 YOLO11l(COCO 면적 구간). 디코드 단계를 FP32로 둔 INT8(헤드 FP32, 디코드 FP32)은 크기와 무관하지만, 좌표 텐서를 양자화한 조건(가중치 FP32)과 박스 정규화를 적용한 전체 INT8은 작은 객체일수록 크게 잃는다. 수염은 짝지은 부트스트랩 95% 신뢰구간이다.

**보정 방법.** MinMax 대신 ONNX Runtime의 Percentile(99.999%, 양측)과 Entropy(KL 탐색, 2,048 bin)로 보정해 두 질문을 확인하였다.
- 전체 INT8은 두 방법 모두에서 두 검출기가 붕괴하였다(mAP 0). 두 방법도 점수와 좌표가 섞인 텐서의 범위를 거의 줄이지 않아 척도가 2.50–3.58로 남았고, 식 (6)대로 점수가 사라졌다.
- 헤드 제외 INT8에서는 Percentile이 유지율을 높였다(YOLO26-n 97.0% → 98.4%, YOLOv8s 98.7% → 98.99%). Entropy는 오히려 낮았다(YOLO26-n 95.6%, YOLOv8s 92.9%). ONNX Runtime의 Entropy 범위는 0을 중심으로 대칭이라, 음수가 적은 활성값에서는 UINT8 단계의 일부를 쓰지 못한다.
- YOLO26-n의 Percentile 변형은 신뢰구간(97.8–98.9%)이 기준 아래여서 불합격이다. YOLOv8s의 Percentile 변형은 신뢰구간(98.6–99.3%)이 기준을 포함해 MinMax와 같이 보류였다(25프레임 블록 부트스트랩도 98.6–99.4%). 따라서 보정 방법을 바꾸어도 99% 기준을 확실히 넘는 구성은 없었다.

**Table 5. 양자화 요인 분리와 완화책.** 값은 test 전체의 mAP@0.5:0.95 유지율(%)이다. 가중치 전용·활성값 전용 변형은 전체 INT8 QDQ 그래프에서 표시한 부분만 양자화하고 나머지를 FP32로 되돌렸다. 붕괴 텐서는 단일 텐서 선별 검사에서 단독으로 mAP 0을 만든 텐서이다(YOLO26-n 5개, YOLOv8s 1개). 좌표 정규화 변형은 전체 그래프 INT8(MinMax)이고, 보정 방법 행은 MinMax 대신 표시한 방법을 썼다. YOLOv8s의 헤드 FP32·Percentile 값은 98.99%(신뢰구간 98.6–99.3%)로 99% 기준 보류이다.
...
| W8A8, 좌표 정규화 | 사후 정규화 (vanilla rescaling) | 42.4 | 34.1 |
| | 사전 정규화 (Ultralytics TF 내보내기) | 50.9 | 55.0 |
| | stride를 회귀 합성곱에 접음 (RN 구조) | 51.0 | – |
| W8A8, 보정 방법 | 전체 그래프, Percentile | 0 | 0 |
| | 전체 그래프, Entropy | 0 | 0 |
| | 헤드 전체 FP32, Percentile | 98.4 | 99.0 |
| | 헤드 전체 FP32, Entropy | 95.6 | 92.9 |

**종단 정확도로의 전파.** 구성요소의 손실이 최종 인식 결과까지 어떻게 전파되는지 두 검출기에서 Table 6으로 확인하였다. YOLOv8s는 공개 시연의 배포 경로와 같이 신뢰도 0.1 이상의 검출에 클래스별 NMS(IoU 0.45)를 적용한 뒤 같은 추적기와 인식기를 거쳤다.
- 인식기의 정밀도(FP32·FP16·INT8)는 종단 정답률을 바꾸지 않았다(유지율 99.7–100.0%). FP16 검출기도 두 모델 모두 99.8% 이상을 유지하였다.
- 헤드 제외 INT8 검출기의 종단 유지율은 YOLO26-n 94.2%(신뢰구간 92.8–95.7%), YOLOv8s 98.2%(97.2–99.3%)였다. 두 모델 모두 검출 mAP 유지율(97.0%, 98.7%)보다 낮았다. 손실은 YOLO26-n에서 두 배 가까이 컸고, YOLOv8s에서는 1.3배로 작았으며 신뢰구간이 mAP 유지율과 겹쳤다.
- 대응된 객체의 인식 정확도는 FP32와 거의 같았다(YOLO26-n 86.2% 대 86.5%, YOLOv8s 86.0% 대 85.8%). 따라서 손실은 대응된 객체 수가 줄어든 데서 온다(YOLO26-n −5.4%, YOLOv8s −1.9%).
- mAP는 모든 신뢰도 임계값에 걸쳐 정확도를 평균하지만, 파이프라인은 추적 임계값(0.25)에서 동작한다. INT8이 신뢰도를 전반적으로 낮추면 이 임계값을 넘는 검출이 줄고, 그 손실이 그대로 최종 결과로 이어진다.
- 따라서 파이프라인에서 양자화 여부를 판정할 때는 검출 mAP만 보지 말고, 운영 임계값에서의 종단 지표도 함께 확인해야 한다. Fig. 4는 이 차이를 두 test 프레임에서 보여 준다.

**Table 6. 검출→추적→인식 종단 정확도.** test 전체에서 세부 라벨이 있는 GT 6,771개를 기준으로 한다.

| 모델 | 검출기 | 인식기 | 대응 | 조건부 정확도 (%) | 종단 정답률 (%) | 유지율 (%) [95% CI] |
| :--- | :--- | :--- | ---: | ---: | ---: | ---: |
| YOLO26-n | FP32 | FP32 | 2,936 | 86.5 | 37.5 | 100 |
...
| | INT8 전체 | 모든 변형 | 0 | – | 0 | 0 |
| YOLOv8s | FP32 | FP32 | 3,462 | 85.8 | 43.9 | 100 |
| | FP32 | INT8 전체 | 3,462 | 85.6 | 43.8 | 99.8 [99.5–100.1] |
| | FP16 | FP32 | 3,463 | 85.8 | 43.9 | 100.0 [99.5–100.5] |
| | INT8 헤드 제외 | FP32 | 3,397 | 86.0 | 43.1 | 98.2 [97.2–99.3] |
| | INT8 헤드 제외 | INT8 전체 | 3,397 | 85.8 | 43.0 | 98.0 [97.0–99.0] |
| | INT8 전체 | 모든 변형 | 0 | – | 0 | 0 |

주. 대응: 같은 상위 클래스의 추적 상자와 IoU 0.5 이상으로 대응된 GT 수. 조건부 정확도: 대응된 GT 가운데 세부 클래스를 맞힌 비율. 종단 정답률: 전체 GT 가운데 대응되고 세부 클래스까지 맞힌 비율. 유지율: 같은 모델의 FP32 검출기 + FP32 인식기 대비. 대괄호는 프레임 부트스트랩 95% 신뢰구간이다.

![Fig. 4](figures/fig10_qualitative.png)

**Fig. 4. YOLO26-n 정밀도별 검출 예(신뢰도 0.25 이상).** 흰 점선은 GT, 색 상자는 검출(하늘색: 표지판, 주황: 신호등)이며, 숫자는 신뢰도이다. 프레임 선택 규칙: GT 객체 수가 가장 많은 주간 test 프레임과, 그로부터 100프레임 이상 떨어진 프레임 가운데 GT 수가 가장 많은 프레임. INT8 전체는 두 프레임 모두 검출이 없다. 헤드 제외 INT8은 대부분의 검출을 유지하지만, 신뢰도가 임계값 근처인 검출은 사라지거나 바뀐다.

### 4.3. RQ2: Runtime-Dependent Latency and Its Causes

Table 7은 기기 A(Windows)에서 같은 모델 파일의 배치 1 추론 지연이다. 결과는 세 가지로 요약되며, 기기 B(Mac)에서의 재현 여부는 이 절의 끝에서 Table 9와 Fig. 5로 정리한다.

**Table 7. 기기 A의 실행 환경별 추론 지연.** 값은 평균 / p90 (ms)이다. 1,024회 측정(WebGPU INT8만 128회). INT8은 INT32 bias 변형이며, 1.22 WebGPU에서는 실행에 실패하였다. 인식기 WASM은 1스레드이다.
...
**두 번째 기기에서의 재현.** 같은 모델 파일과 절차로 기기 B(Apple M2 Pro)에서 다시 측정하였다. Table 9는 두 기기에서 정밀도의 효과를 같은 실행 환경 안의 비율로 비교하고, Fig. 5는 대표 조건의 절대 지연을 두 기기에 나란히 보여 준다.
...
![Fig. 5](figures/fig11_runtime_latency_devices.png)

**Fig. 5. 두 기기의 검출기 배치 1 추론 지연.** 로그 축이며, 막대는 평균, 수염은 p90이다. 대표 조건만 표시하였다: 네이티브 ORT CPU 4스레드, WASM 4스레드, WebGPU(ORT-Web 1.30)의 FP32·FP16과 INT8(헤드 제외, INT32 bias). 파선은 30 FPS(33.3 ms), 점선은 15 FPS(66.7 ms) 예산이다. 기기 A의 전체 조건(1스레드, ORT-Web 1.22, 인식기)은 Table 7에 있다.

### 4.4. RQ3: Component-Wise Runtime Placement

Table 10은 한 브라우저 페이지에서 검출기(YOLO26-n)와 인식기를 각각 지정한 실행 환경에 배치한 파이프라인의 프레임당 지연이고, Fig. 6은 주요 배치의 단계별 지연을 두 기기에 나란히 보여 준다. 반복 측정한 배치는 같은 회차에 측정한 배치끼리 짝지어 차이를 비교하였다.

**Table 10. 브라우저 파이프라인 배치별 프레임당 검출–인식 지연.** 값은 ms이며, 판정 열은 "기기 A / 기기 B"이다.
...
주. 반복 측정한 배치(기기 A 7개, 기기 B 4개; † 표시)는 브라우저 실행 5회(각 512프레임) 결과의 중앙값이고, 대괄호는 실행 간 [최소–최대]이다. 나머지는 1회(512프레임) 측정이다. 기준 판정은 실행별 p90의 중앙값으로 하였다. 5회의 2,560프레임을 합친 p90으로 판정해도 모든 배치에서 결과가 같았다. 판정 열은 "기기 A / 기기 B"이다. WASM은 교차 출처 격리 페이지의 4스레드이다(1T 표시 행 제외). 종단 유지율은 같은 정밀도 조합의 Table 6 값(ONNX Runtime CPU)이다. 브라우저 실행 환경과의 수치 일치를 확인한 범위는 4.3절에 있다.
...
![Fig. 6](figures/fig12_pipeline_devices.png)

**Fig. 6. 두 기기의 주요 파이프라인 배치별 단계 지연.** 배치마다 위 막대는 기기 A, 아래 막대는 기기 B이다. 막대는 실행별 평균의 중앙값, 수염은 실행 간 범위이고, 숫자는 합계와 p90의 중앙값 및 실행 횟수(n)이다. 파선은 30 FPS, 점선은 15 FPS 예산이다. WASM 1스레드 배치와 WebGPU INT8 배치(기기 A 897 ms, 기기 B 225 ms)는 Table 10에만 표시하였다.
...
주. 유지율은 FP32 대비 mAP@0.5:0.95이고, 대괄호는 이미지 단위 부트스트랩 95% 신뢰구간이다. INT8 셀의 "a / b"는 INT32 bias / FP32 bias 변형의 값이다. 신뢰구간은 INT32 bias 변형의 것이다. 진단 변형은 가중치를 FP32로 두고, 표시한 활성값만 양자화하였다.
...
**Table 12. YOLO11l 실행 환경별 추론 지연.** 값은 평균 / p90 (ms)이다.
...
주. 1,024회 측정(WebGPU INT8만 128회)이다. ORT-Web 1.22 WebGPU의 INT8은 FP32 bias 변형이다. 기기 A에서 측정 중 기록한 CPU 부하가 배경 프로세스와 겹친 네 측정(CPU 4T의 FP32·FP16·INT8 전체, WASM 4T의 FP32)은 부하가 가라앉은 뒤 다시 측정한 값이다. 처음 값과의 차이는 10% 이내였다. 기기 B는 대표 조건만 측정하였다(–: 측정 안 함).
...
- **mAP 유지율은 파이프라인 손실을 과소평가한다.** INT8의 손실은 신뢰도 분포의 이동으로 나타났고, 운영 임계값에서 종단 손실이 mAP 손실보다 컸다(YOLO26-n 약 두 배, YOLOv8s 1.3배; 4.2절). 식 (1)의 제약을 mAP로 대신하면 실제보다 관대한 결정을 내리게 된다.
- **완화책의 선택도 실행 환경에 달려 있다.** 좌표 정규화는 붕괴를 없애지만, 8비트 좌표의 해상도 한계(640 입력에서 약 2.5 px) 때문에 작은 객체가 많은 도로 워크로드에서 손실이 컸다. 정수 전용 가속기처럼 그래프 전체를 정수로 실행해야 하는 배포에서는 이 한계를 피하기 어렵지만, ONNX QDQ 그래프를 실행하는 브라우저 런타임에서는 디코드 단계나 헤드를 FP32로 남길 수 있다(유지율 92.8–98.7%). 다만 정규화한 모델도 `QuantizeLinear` 수가 원래 전체 INT8과 같아(YOLOv8s 240–241개, YOLO26-n 386–388개), WebGPU에서의 CPU 배치 문제는 그대로 남는다. 모델 쪽 정확도 문제를 푸는 것과 실행 환경 쪽 효율 문제를 푸는 것은 별개이다.
...
- **측정 범위:** 파이프라인 지연에는 추적기, 캔버스 렌더링, 카메라·영상 디코딩이 빠져 있다. 30 FPS 기준 통과는 이 범위에 한정된다. 종단 정답률은 프레임 단위의 검출·인식 결과로 계산했고, 추적 정확도(정체성 유지)는 수동 정체성 주석이 없어 평가하지 않았다. 종단 정확도는 도로 워크로드의 두 검출기에서만 측정했으며, YOLOv8s의 NMS는 배포 설정(IoU 0.45)을 따랐다.
...
- **완화책과 보정 실험의 범위:** 좌표 정규화와 보정 방법 비교는 도로 워크로드의 두 검출기에서만 수행하였다. Moon 등의 RN은 DFL이 없는 YOLO26-n에서만 같은 구조로 옮길 수 있었고, 640 배율 복원은 후처리에서 하였다. ONNX Runtime의 Entropy 보정은 대칭 범위를 쓰며, 여기서는 bin 수를 2,048로 늘렸다. 정규화 모델의 지연은 사전 규칙(유지율 90% 이상일 때만 측정)에 따라 측정하지 않았다.
...
- **알려진 혼합 범위 실패의 재현(기여 1).** Moon 등 [10]이 모바일 TFLite에서 보고한 박스 좌표·클래스 점수의 동적 범위 불일치 실패는 YOLOv8s, YOLO26-n, YOLO11l의 ONNX QDQ 그래프에서도 재현되었다. YOLO11l에서는 측정 전에 공개한 예측이 모두 맞았다.
- **텐서 수준의 원인 분리(기여 2).** 실패에는 가중치 양자화가 필요하지 않았고, 좌표와 점수를 한 척도로 담는 디코드 단계 텐서 하나의 양자화로 충분하였다. 단일 척도로는 전체 좌표 범위와 낮은 신뢰도의 점수를 함께 보존할 수 없다.
- **남는 손실과 종단 전파(기여 3).** 알려진 좌표 정규화는 붕괴를 없앴지만 8비트 좌표 반올림 때문에 유지율이 34–55%에 그쳤고, 그 손실은 작은 객체에 몰렸다. 헤드를 FP32로 두어 실패를 피해도 도로 워크로드의 두 검출기는 사전 기준 99%에 이르지 못했고, 손실은 종단 정답률에서 더 커졌다(YOLO26-n 약 두 배). 반면 작은 인식기는 전체 INT8에서도 손실이 없었다. 양자화는 구성요소 단위로, 운영 조건의 종단 지표로 검증해야 한다.
- **연산자 배치와 지연(기여 4).** INT8과 FP16의 속도 효과는 실행 환경과 런타임 버전에 따라 방향이 바뀌었고, 그 원인은 연산자 배치로 설명되었다. 검출기를 WebGPU에, 인식기를 WASM에 둔 구성요소별 배치가 가장 빨랐다.
- **기기 간 비교(기여 5).** 연산자 배치와 그에 따른 효과의 방향은 시험한 두 기기에서 같았고 구성요소별 배치도 두 기기에서 가장 빨랐지만, WASM에서 INT8의 이득은 두 기기에서 달랐다. "양자화하면 엣지에서 빨라진다"는 가정은 목표 실행 환경, 런타임 버전, 기기를 명시할 때만 성립한다.

정리하면, 양자화 범위를 정하는 원칙(점수와 좌표가 섞인 텐서를 피하고, 작은 객체가 많으면 좌표 텐서도 FP32로 남긴다)은 평가한 검출기 그래프들에서 일반화되지만, 남는 손실의 크기와 실제 속도 이득은 워크로드와 기기마다 확인해야 한다.
...
- **양자화 방법:** 텐서별 활성값 척도를 쓰는 정적 PTQ(MinMax·Percentile·Entropy 보정)와 알려진 좌표 정규화만 평가하였다. 두 도로 검출기에서 사전 기준 99%를 확실히 넘는 INT8 구성은 찾지 못했다.
...
- **종단 평가:** 종단 정확도는 한 워크로드(도로 파이프라인)의 두 검출기에서만 측정하였고, 수동 정체성 주석이 없어 추적 품질(정체성 유지)은 평가하지 않았다.
...
- **양자화 방법:** 채널별 활성값 척도, 박스와 점수를 서로 다른 출력으로 나눈 그래프, 양자화 인지 학습(QAT)으로 작은 객체의 좌표 해상도 손실과 헤드 분기의 활성값 손실을 줄여 99% 기준을 충족하는 INT8 검출기를 탐색한다.
